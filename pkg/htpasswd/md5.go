...
func compareMd5Hash(plaintext []byte, hash []byte) bool {
	...
	salt := parts[2]
	if subtle.ConstantTimeCompare(hash, md5Crypt(plaintext, salt, magic)) != 1 {
		return false
	}
	return true
}

// Source: https://github.com/abbot/go-http-auth/blob/master/md5crypt.go
